      ...
      <div className="px-5 py-4 flex items-center justify-between">
        ...
        {quests.length > 0 && (
          <span className="text-xs font-medium px-2 py-0.5 rounded-full"
            style={{ background: doneCount === quests.length ? '#d1fae5' : '#f3f4f6', color: doneCount === quests.length ? '#059669' : '#9ca3af' }}>
        ...
        <AnimatePresence initial={false}>
          {quests.map((quest) => (
            <motion.div key={quest.id} layout
              ...
              <SwipeableRow onDelete={() => deleteQuest(quest.id)}>
                <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-50 group">
                  <button
                    onClick={(e) => {
                    ...
                    style={{ borderColor: quest.completed_today ? '#10b981' : '#d1d5db', background: quest.completed_today ? '#10b981' : 'white', cursor: quest.completed_today ? 'default' : 'pointer' }}
                  >
                  ...
                  <span className="flex-1 text-sm" style={{ color: quest.completed_today ? '#9ca3af' : '#111827', textDecoration: quest.completed_today ? 'line-through' : 'none' }}>
                    {quest.quest_text}
                  </span>

                  <span className="text-xs font-medium flex-shrink-0 text-gray-300">+{QUEST_XP}</span>
          ...
          <form onSubmit={handleAdd} className="flex items-center gap-3 px-5 py-3.5">
            ...
            {newText.trim() && (
              <button type="submit" className="flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full"
                style={{ background: '#f3f4f6', color: '#6b7280' }}>Add</button>
            )}
          </form>
        )}
      </div>
